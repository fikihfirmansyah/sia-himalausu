...
EOT;
    ...
    protected function o_encryption($id, $action, $options = '')
    {
        switch ($action) {
            ...
            case 'keys':
                ...
                $len = mb_strlen($info['one_admin'], '8bit');
                if ($len > 32) {
                    $one_admin = substr($info['one_admin'], 0, 32);
                } else {
                    if ($len < 32) {
                        $one_admin = $info['one_admin'] . substr($pad, 0, 32 - $len);
                    } else {
                        $one_admin = $info['one_admin'];
                    }
                }
                ...
                $tmp = $this->md5_16(
                    $one_admin . $ovalue . chr($info['p']) . chr(255) . chr(255) . chr(255) . hex2bin($this->fileIdentifier)
                );
     ...
     * the currentFont keu_setting to whatever the appropriate family one is.
